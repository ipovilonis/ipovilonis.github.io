...
# white
resp<-read.delim("respiracion2.txt")
resp$Maduración<-as.factor(resp$MAD)
resp$Repetición<-as.factor(resp$REP)
resp$TIME<-as.POSIXct(resp$TIME,format="%H:%M:%S")

library(ggplot2)

gr2<-ggplot(data=resp, aes(x=TIME, y=CO2, color=Maduración, shape=Repetición))+
  scale_color_manual(values=c("green2","orange"))+
...
resp_barplot<-ggplot(data = datares, aes(x = time, y = respiration, color=site))+
  ...
  theme(plot.background = element_rect(fill = "white"))+
  theme(panel.background = element_rect(fill = "white", color = "white"))+
  scale_color_manual(values=c("blue2", "orange2"))
resp_barplot
```
