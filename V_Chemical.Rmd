...
## Metodología
# Se muestrearon 10 frutos por genotipo de *Hexachlamys edulis* en 3 poblaciones de Entre Ríos: Concordia, PN El PN El Palmar y Gualeguaychú.
...
library(readxl)
datavq<-read_excel("database.xlsx", sheet="V varqui")
datavq$year<-as.factor(datavq$year)
datavq$phenotype<-as.factor(datavq$phenotype)
datavq$site<-factor(datavq$site,levels=c("Concordia", "PN El Palmar", "Gualeguaychú"))
...
library(readxl)
dataqui<-read_excel("database.xlsx", sheet="V varqui")

dataqui$year<-as.factor(dataqui$year)
dataqui$site<-factor(dataqui$site,levels=c("Concordia", "PN El Palmar","Gualeguaychú"))
...
### Individuos del PN El Palmar en ambos años.
```{r  , echo=FALSE, fig.align='center', fig.asp=0.4, fig.width=10, fig.height=8}
knitr::opts_chunk$set(warning = FALSE, message = FALSE, echo = FALSE)

Palmar<-subset(datavq, site == "PN El Palmar")
...
g3<-ggplot(data=estpal, aes(x=phenotype, y=fen, form=phenotype, fill=phenotype)) +  
  ...
  geom_errorbar(aes(ymin=fen-sd, ymax=fen+sd),
                width=.2, position=position_dodge(.9))+
  facet_grid(.~year)+
  labs(x="PN El Palmar", y="fenoles (mg ácido tánico/100g de peso fresco)")+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5, size = rel(1.5))) +
  theme(legend.title=element_blank())+
  theme(legend.position='none')+
...
guale<-subset(datavq, site == "Gualeguaychú")
...
library(readxl)
dataacso<-read_excel("database.xlsx", sheet="V acidezsolidos")
dataacso$year<-as.factor(dataacso$year)
dataacso$phenotype<-as.factor(dataacso$phenotype)
dataacso$site<-factor(dataacso$site,levels=c("Concordia", "PN El Palmar","Gualeguaychú"))
...
gcor<-ggplot(data=dataacso, aes(x=ac, y=brix, color=year, shape=phenotype))+
  ...
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5, size = rel(1.5))) +
  scale_color_manual(values = c("coral1","green2","yellow")) +
  theme(legend.title=element_blank())+
