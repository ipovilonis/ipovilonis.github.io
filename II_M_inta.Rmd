...
#Create object "db_met_mor" which contains climate data
db_hel_mor <- read_excel("database.xlsx", sheet = "II_M_heladas")
...
gg_month_mor_temp_min_0<-ggplot(datos_unidos, aes(x = month, y = Mean_min, color = year, fill = year, width = 1)) +
    ...
    annotate("text", x = 3, y = medias_marzo$Mean + c(-1.8, 0, 0), label = c("b", "b", "a"), 
             color = palette, size = 8, vjust = -1) +
    # Añadir letras de significancia para cada año en abril
    annotate("text", x = 4, y = medias_abril$Mean + c(0, 0.5, -1.8), label = c("a", "b", "b"), 
             color = palette, size = 8, vjust = -1) +
    # Añadir letras de significancia para cada año en mayo
    annotate("text", x = 5, y = medias_mayo$Mean + c(0, 0, -1.8), label = c("a", "ab", "b"), 
             color = palette, size = 8, vjust = -1) +
    # Añadir letras de significancia para cada año en junio
    annotate("text", x = 6, y = medias_junio$Mean, label = c("b", "a", "a"), 
             color = palette, size = 8, vjust = -1) +
    # Añadir letras de significancia para cada año en septiembre
    annotate("text", x = 9, y = medias_septiembre$Mean + c(0.5, 0, -2), label = c("a", "b", "b"), 
             color = palette, size = 8, vjust = -1)
...
gg_month_mor_temp_min <- gg_month_mor_temp_min_0 +
  # ... (tu código previo para las barras y otras configuraciones)
  ...
  annotate("text", 
           x = c(get_x_positions(6), get_x_positions(7), get_x_positions(9)),
           y = c(heladas_junio$Heladas, heladas_julio$Heladas, heladas_septiembre$Heladas) + 0.5,
           label = unlist(labels),
