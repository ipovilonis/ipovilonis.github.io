...
library(glmmTMB)
modelo_sec_uba_aao_2 <- glmmTMB(aao/100 ~ Tratamiento + (1|Concentracion) + (1|Dia), family = beta_family(link = "logit"), data = datosaao_uba)

# modelo_sec_uba_aao_3 <- glmmTMB(aao/100 ~ Tratamiento + (1|Concentracion) + (1|Dia), family = beta_family(link = "cloglog"), data = datosaao_uba)
# anova(modelo_sec_uba_aao_2,modelo_sec_uba_aao_3)
...
HV_H_modelo_sec_uba_aao2<-ggplot(H_modelo_sec_uba_aao) +
  aes(predichos, residuos) +
  geom_hline(yintercept = 0, colour="white", size=2) +
  geom_point(colour="white")+
  theme_classic()+
...
confint(Comp_modelo_sec_uba_aao_2)

# Resumen de los resultados
summary_results <- summary(Comp_modelo_sec_uba_aao_2)

# Si necesitas calcular el desvío estándar a partir del error estándar
n <- summary_results$emmean$df + 1  # tamaño de muestra (ajusta según tu modelo)
desviaciones_estandar <- summary_results$contrasts[, "SE"] * sqrt(n)


gg_Comp_modelo_sec_uba_aao_2 <- plot(Comp_modelo_sec_uba_aao_2, comparisons = TRUE)+ 
labs(x = "Actividad Antioxidante [%/100]", y ="Ubajay")+
  annotate("text", x = 0.3628177 , y = 1.2 , label = "a", size=6)+
  annotate("text", x = 0.2292810 , y = 2.2 , label = "b", size=6)+
  annotate("text", x = 0.3012312 , y = 3.2 , label = "a", size=6)+
  theme_classic()+
  theme(panel.grid.major.y = element_line(size=0.05, color='black'))+
  theme(panel.grid.major.x = element_blank())+
  theme(text = element_text(size=32, color='black'))+
...
response_data <- data.frame(confint(Comp_modelo_sec_uba_aao_2$emmeans))
...
sec_uba_aao_comp<-ggplot(response_data, aes(x=Tratamiento, y=response*100, fill = Tratamiento)) +
  ...
  geom_errorbar(aes(ymin = response*100-response_data$SE *100, ymax = response*100+SE*100), width = 0.2, position = "dodge", color='black', size = 1)+
  ...
  theme(legend.title=element_blank())+
  theme(legend.position = "bottom")+
  scale_y_continuous(limits = c(0, 0.99*100), breaks = seq(0, 1*100, 0.10*100))+
...
response_data_cal <- data.frame(Comp_modelo_sec_cal_aao_2$emmeans)

response_data_cal$Tratamiento <- as.factor(response_data_cal$Tratamiento)

library(ggplot2)
sec_cal_aao_comp<-ggplot(response_data_cal, aes(x=Tratamiento, y=response*100, fill = Tratamiento)) +
  geom_bar(stat = "identity", width=.4, color='black', size = 1) +
  ...
  theme(legend.position = "bottom")+
  scale_y_continuous(limits = c(0, 0.99*100), breaks = seq(0, 1*100, 0.10*100))+
  annotate("text", y = 0.8011113  *100+0.1*0.8011113 *100 , x = 1.05 , label = "a",size=10)+
  annotate("text", y = 0.4876685   *100+0.1*0.4876685  *100 , x = 2.05 , label = "b",size=10)+
  annotate("text", y = 0.7317074   *100+0.1*0.7317074 *100 , x = 3.05 , label = "a",size=10)
sec_cal_aao_comp
```
...
sec_uba_car_comp<-ggplot(response_data2, aes(x=Tratamiento, y=response, fill = Tratamiento)) +
  ...
  annotate("text", y = 259.34016 +20  , x = 1.2 , label = "a",size=10)+
  annotate("text", y = 80.24991  +20   , x = 2.2 , label = "b",size=10)+
  annotate("text", y = 96.00817 +20   , x = 3.2 , label = "b",size=10)
sec_uba_car_comp
...
datosanto$Dia<-as.factor(datosanto$Dia)
```
...
library(ggplot2)
sec_cal_anto<-ggplot(tabla_sec_anto, aes(x=Dia, y=Mean, fill=Tratamiento, width=.5)) +
  stat_summary(fun = "mean", size = 1, geom = "bar",position=position_dodge(0.5), color="black") +
  geom_errorbar(aes(ymin=Mean-sd, ymax=Mean+sd),
                width=.1, position=position_dodge(0.5), size=1)+
  scale_fill_manual(values=c("yellow3","lightblue","coral","red4"))+
  labs(y="Antocianinas [mg / 100 g PS]", x="Día")+
  # facet_grid(.~Tratamiento)+
  theme_classic()+
  theme(panel.grid.major.y = element_line(size=0.05, color='black'))+
  theme(panel.grid.major.x = element_blank())+
...
labs(x = "Antocianinas [mg / 100 g PS]", y ="")+
  annotate("text", x = 1690.368 , y = 1.4 , label = "a",size=6)+
  annotate("text", x = 1349.826 , y = 2.4 , label = "ab",size=6)+
  annotate("text", x = 1013.290 , y = 3.4 , label = "b",size=6)+
  theme_classic()+
  theme(panel.grid.major.y = element_line(size=0.05, color='black'))+
  theme(panel.grid.major.x = element_blank())+
  theme(text = element_text(size=32, color='black'))+
  theme(axis.text = element_text(color='black'))+
  scale_x_continuous(limits = c(600, 2500), breaks = seq(600, 2500, 200))+
...
response_data_cal_anto <- data.frame(Comp_modelo_sec_cal_anto$emmeans)

response_data_cal_anto$Tratamiento <- as.factor(response_data_cal_anto$Tratamiento)

library(ggplot2)
sec_cal_aao_comp<-ggplot(response_data_cal_anto, aes(x=Tratamiento, y=response, fill = Tratamiento)) +
  geom_bar(stat = "identity", width=.4, color='black', size = 1) +
  geom_errorbar(aes(ymin = response-SE, ymax = response+SE), width = 0.2, position = "dodge", color='black', size = 1)+
  scale_fill_manual(values=c("lightblue","coral","red4"))+
  labs(y="Antocianinas [mg / 100 g PS]", x="")+
  labs(x="", x="")+ #mg peso seco / ml
  theme_classic()+
  theme(panel.grid.major.y = element_line(size=0.05, color='black'))+
  theme(panel.grid.major.x = element_blank())+
  ...
  theme(legend.title=element_blank())+
  theme(legend.position = "bottom")+
  scale_y_continuous(limits = c(00, 2200), breaks = seq(00, 2200, 200))+
  annotate("text", y = 1690.368+0.05*1349.826 , x = 1.05 , label = "a",size=10)+
  annotate("text", y = 1349.826+0.05*1349.826 , x = 2.05 , label = "ab",size=10)+
  annotate("text", y = 1013.290+0.05*1349.826 , x = 3.05 , label = "b",size=10)
sec_cal_aao_comp
```
...
gm_cor_sec_cal <- ggplot(correlaciones_cal, aes(x = Antocianinas, y = aao)) +
  ...
  theme(panel.grid.major.y = element_line(color="black", size=0.05))+
  theme(panel.grid.major.x = element_blank())+
  theme(text = element_text(size=31, color='black'))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color='black'))+
  theme(axis.text.y = element_text(angle = 0, hjust = 1, color='black'))+
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 20))+
