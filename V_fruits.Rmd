...
resumen_fisico <- frutos_fisico %>%
  dplyr::group_by(phenotype) %>%
  dplyr::summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>%
  dplyr::ungroup()


resumen_color <- frutos_color %>%
  dplyr::group_by(phenotype) %>%
  dplyr::summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>%
  dplyr::ungroup()


resumen_quimico <- frutos_quimico %>%
  dplyr::group_by(phenotype) %>%
  dplyr::summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>%
  dplyr::ungroup()


tabla_unida <- resumen_fisico %>%
  dplyr::full_join(resumen_color, by = "phenotype") %>%
  dplyr::full_join(resumen_quimico, by = "phenotype")

tabla_unida <- tabla_unida[,-c(10,13,14,18:22,33:35)]
...
tabla_combinada_ordenada <- tabla_combinada_ordenada %>%
   dplyr::mutate(`year.site` = str_c(site, year, sep = "."))
...
# # hulls <- pca_scores %>%
# #   dplyr::group_by(phenotype) %>%
# #   dplyr::slice(chull(Dim.1, Dim.2)) # Calcula el contorno convexo
# 
...
# 1. Clustering Jerárquico (HCA) usando los scores del PCA
# Usaremos los primeros 3 componentes principales
dist_matrix <- dist(pca_scores[,1:3], method = "euclidean")
hclust_result <- hclust(dist_matrix, method = "ward.D2")

# Determinar el número óptimo de clusters usando el método del codo
# library(factoextra)
# fviz_nbclust(pca_scores[,1:3], hcut, method = "wss") +
#   labs(title = "Método del codo para número óptimo de clusters")

# Cortar el dendrograma en k clusters (7)
k_clusters <- 7
clusters_hca <- cutree(hclust_result, k = k_clusters)

# Agregar los clusters al dataframe de scores
pca_scores$cluster_hca <- as.factor(clusters_hca)

# 2. Clustering con k-means
set.seed(123) # Para reproducibilidad
kmeans_result <- kmeans(pca_scores[,1:3], centers = k_clusters, nstart = 25)
pca_scores$cluster_kmeans <- as.factor(kmeans_result$cluster)

# Visualización del HCA en 2D (PC1 vs PC2)
gg_hca <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = cluster_hca)) +
  geom_point(size = 5) +
  ...
  labs(title = "Clustering Jerárquico (HCA)",
       x = "Componente Principal 1 (35,7 %)",
       y = "Componente Principal 2 (18,2 %)") +
  theme_classic() +
  theme(legend.position = "bottom",
...
# Visualización del k-means en 2D (PC1 vs PC2)
gg_kmeans <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = cluster_kmeans)) +
  geom_point(size = 5) +
  scale_color_manual(values = c(
  "#FF0000",  # Rojo puro
  "#00FF00",  # Verde lima
...
p <- layout(p, 
            scene = list(xaxis = list(title = 'PC1'),
                         yaxis = list(title = 'PC2'),
                         zaxis = list(title = 'PC3')),
            title = "Clusters k-means en 3D",
            legend = list(title = list(text = "Clusters")))

# Mostrar el gráfico
print(p)

# Resumen de las características por cluster (usando k-means)
cluster_summary <- pca_scores %>%
  dplyr::group_by(cluster_kmeans) %>%
  dplyr::summarise(across(c(PC1, PC2, PC3), 
                  list(mean = mean, sd = sd),
                  .names = "{.col}_{.fn}")) %>%
  dplyr::left_join(
    tabla_pca %>% 
      dplyr::mutate(cluster_kmeans = pca_scores$cluster_kmeans) %>%
      dplyr::group_by(cluster_kmeans) %>%
...
dist_matrix <- dist(pca_scores[, c("PC1", "PC2", "PC3")], method = "euclidean")
hclust_result <- hclust(dist_matrix, method = "ward.D2")
...
data_labels <- data.frame(
  x = pca_result$ind$coord[, 1],    # Coordenadas Dim.1 (PC1)
  y = pca_result$ind$coord[, 2],    # Coordenadas Dim.2 (PC2)
...
# 1. Clustering Jerárquico (HCA)
dist_matrix <- dist(pca_scores[, c("PC1", "PC2", "PC3")], method = "euclidean")
hclust_result <- hclust(dist_matrix, method = "ward.D2")

# # Método del codo
# fviz_nbclust(pca_scores[, c("PC1", "PC2", "PC3")], hcut, method = "wss") +
#   labs(title = "Método del codo para número óptimo de clusters")

# Dendrograma con fenotipos y colores (solución robusta)
# Cortar en 7 clusters (cambiamos de 3 a 7)
k_clusters <- 7
clusters_hca <- cutree(hclust_result, k = k_clusters)
pca_scores$cluster_hca <- as.factor(clusters_hca)

# Definir paleta de 7 colores
paleta_7 <- c(
  "#FF0000",  # Rojo puro
  "#00FF00",  # Verde lima
  "#0000FF",  # Azul puro
...
# Dendrograma con R base, etiquetas de fenotipos y rectángulos para 7 clusters
plot(hclust_result, 
     labels = pca_scores$phenotype, 
     main = "Dendrograma de Clustering Jerárquico (k = 7)", 
     xlab = "", 
     sub = "", 
     cex = 1.2,         # Tamaño de las etiquetas de fenotipos (aumentado de 0.6 a 1.2)
     cex.main = 1.5,    # Tamaño del título (1.5 veces el tamaño base)
     hang = -1)         # Alinea etiquetas en la base
rect.hclust(hclust_result, k = 7, border = paleta_7)

# 2. Clustering con k-means
set.seed(123)
kmeans_result <- kmeans(pca_scores[, c("PC1", "PC2", "PC3")], centers = k_clusters, nstart = 25)
pca_scores$cluster_kmeans <- as.factor(kmeans_result$cluster)

# # Visualización HCA en 2D
# gg_hca <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = cluster_hca)) +
#   geom_point(size = 5) +
#   scale_color_manual(values = paleta_7) +
#   labs(title = "Clustering Jerárquico (HCA)",
#        x = "Componente Principal 1 (35,7 %)",
#        y = "Componente Principal 2 (18,2 %)") +
#   theme_classic() +
...
PCA discriminado por kluster 7
```{r , echo=FALSE, fig.align='center', fig.asp=0.4, fig.width=24, fig.height=16}
knitr::opts_chunk$set(warning = FALSE, message = FALSE, echo = FALSE)
...
# paleta_7 <- c("#FF6B6B", "#00CD99", "#45B7D1", "#96CEB4", "#FFEEAD", "#D4A5A5", "#9B59B6")
paleta_7 <- c(
  "#FF0000",  # Rojo puro
  "#00FFFF",  # Verde lima
  "#FF00FF",  # Azul puro
...
gg_pca_fruits_cluster_hca <- fviz_pca_biplot(pca_result, 
                                geom.ind = "point",
                                pointshape = 21,
                                pointsize = 6,
                                geom.var = c("point", "text"),
                                labelsize = 8,
                                fill.ind = pca_scores$cluster_hca,
                                col.ind = "black",
                                repel = TRUE,
                                addEllipses = FALSE,
                                label = "var",
                                col.var = "black",
                                arrows = TRUE) + 
  scale_fill_manual(values = paleta_7, name = "Clusters HCA") +
  labs(title = "", 
       x = "Componente Principal 1 (28,6 %)",
       y = "Componente Principal 2 (22,5 %)") +
  theme_classic() +
  ...
  geom_text_repel(data = data_labels, 
                  aes(x = x, y = -y, label = label),
                  size = 6, color = "black") +
  coord_cartesian(xlim = c(-5, 6.5), ylim = c(-7.5, 5))
print(gg_pca_fruits_cluster_hca)
...
ploty3D_sf <- plot_ly(data = pca_scores,
             x = ~PC1,
             y = ~PC2,
             z = ~PC3,
             type = 'scatter3d',
...
# Crear tabla de fenotipos por cluster (HCA)
phenotypes_por_cluster_hca <- pca_scores %>%
  dplyr::group_by(cluster_hca) %>%
  dplyr::summarise(Fenotipos = paste(phenotype, collapse = ", ")) %>%
  dplyr::ungroup()

# Mostrar la tabla
print(phenotypes_por_cluster_hca)
```
...
# Generar tabla resumen usando cluster_hca
cluster_summary <- pca_scores %>%
  dplyr::group_by(cluster_hca) %>%
  dplyr::summarise(across(c(PC1, PC2, PC3), 
                   list(mean = mean, sd = sd),
  ...
  left_join(
    tabla_pca %>% 
      dplyr::mutate(cluster_hca = pca_scores$cluster_hca) %>%
      dplyr::group_by(cluster_hca) %>%
      dplyr::summarise(across(where(is.numeric), mean, na.rm = TRUE)),
    by = "cluster_hca"
  )
