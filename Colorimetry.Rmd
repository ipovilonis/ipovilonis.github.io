...
d <- density(icolor_L$color_L)
modes <- function(d){
  i <- which(diff(sign(diff(data$color_L))) < 0) + 1
  data.frame(x = d$x[i], y = d$y[i])
}
micolor_L<-modes(d)[1:170,]
D_i_L<-micolor_L$x[which.max(micolor_L$y)]  
# 51.37369
mmcolor_L<-data[data$mad == "MM", ]

d <- density(mmcolor_L$color_L)
modes <- function(d){
  i <- which(diff(sign(diff(data$color_L))) < 0) + 1
  data.frame(x = d$x[i], y = d$y[i])
}
...
Se encontraron diferencias significativas para todos los estados de madurez en las 3 escalas color_L, a y b. Sólo en la escala b no se diferencia MM de SM.
...
tdplot <-scatterplot3d(data[7:9],
                       ...
                       # type="h",
                       angle = 280)
legend("topright", legend = unique(data$site), col = unique(colors), pch = 16, title = "Sites")
...
tdplot <-scatterplot3d(data[7:9],
                       ...
                       # type="h",
                       angle = 280)
legend("topright", legend = unique(data$mad), col = unique(colors), pch = 16, title = "Maturity")
...
colores<-c("palegreen4","green2","orange","burlywood4")
library(ggplot2)
gcorLabmad<-ggplot(data=data, aes(x=axis_1, y=axis_2, color=mad,shape=site))+
geom_point(size=2)+
...
library(ggplot2)
gcorLabsite<-ggplot(data=data, aes(x=axis_3, y=axis_4, color=site, shape=mad))+
geom_point(size=2)+
labs(x="axis_1", y="axis_2")+
theme_classic()+
scale_color_manual(values=colores)+
