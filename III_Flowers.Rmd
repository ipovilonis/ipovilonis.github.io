...
[*(Yang et al., 1995)*](https://doi.org/10.1016/0168-1923(94)02185-M)
...
mejores_modelos <- results_df_purrr_filtrado %>%
  ...
  slice_max(R2, n = 5) %>% # Seleccionar la fila con el R2 más alto dentro de cada grupo
...
df_grafico <- df_12C %>% 
  mutate(fecha_normalizada = as.Date(mes_dia, format = "%m-%d")) %>% # Crear una columna normalizada
  select(year, fecha_normalizada, Grados_Día_Acumulados) # Seleccionar columnas clave
...
gg_phen_years_simple <- ggplot(df_grafico_completo, aes(x = fecha_normalizada, y = Grados_Día_Acumulados, color = factor(year), group = year)) +
  geom_line(size = 1.5) +
  scale_x_date(
    limits = as.Date(c("2025-08-01", "2025-11-20")),  # Cambiado hasta el 20 de noviembre
    date_labels = "%b-%d", 
    date_breaks = "1 month"
  ) +
  scale_color_manual(values = palette) +
  ...
  geom_hline(aes(yintercept=100), color="grey40")+
  geom_hline(aes(yintercept=200), color="grey40")+
  geom_hline(aes(yintercept=300), color="grey40")+
  geom_hline(aes(yintercept=400), color="grey40")+
  geom_hline(aes(yintercept=500), color="grey40")+
...
gg_phen_years <- gg_phen_years_simple +
  geom_vline(xintercept = as.Date(fecha_inicio_floracion), linetype = "dashed", color = "green4", size = 1.5) +
  geom_vline(xintercept = as.Date(fecha_plena_floracion), linetype = "dashed", color = "#FF0011", size = 1.5) +
  annotate("text", x = as.Date(fecha_inicio_floracion), y = max(df_grafico_completo$Grados_Día_Acumulados), 
           label = "", color = "green4", vjust = -0.5) +
  annotate("text", x = as.Date(fecha_plena_floracion), y = max(df_grafico_completo$Grados_Día_Acumulados), 
           label = "", color = "#FF0011", vjust = -0.5) +
  geom_hline(yintercept = 303.5, linetype = "dashed", color = "green4", size = 1.5) +
  geom_hline(yintercept = 380.2, linetype = "dashed", color = "#FF0011", size = 1.5) +
  # Ajuste para marcas de fecha cada 10 días aproximadamente
  scale_x_date(
    breaks = as.Date(c("2025-08-02", "2025-11-04", "2025-11-13", seq(as.Date("2025-08-02"), as.Date("2025-11-20"), by="10 days"))),
    labels = format(c(as.Date("2025-08-02"), as.Date("2025-11-04"), as.Date("2025-11-13"), seq(as.Date("2025-08-02"), as.Date("2025-11-20"), by="10 days")), "%d-%m")
  )
