...
variables_suplementarias <- datamet %>%
  dplyr::group_by(year, site) %>%
  dplyr::summarise(across(c(temp, prec, hr, wind), mean, na.rm = TRUE)) 

# Unir las variables climáticas promedio a la tabla de respuestas
tabla_merged <- tabla_cca_means %>%
...
tabla_merged <- tabla_merged[,-c(16:19)]
...
biplot_sites_repro <- fviz_pca_biplot(pca_result, 
  ...
  scale_color_manual(values = c("coral1", "burlywood2", "palegreen4"),
                     name = NULL) +
  theme_classic() +
  labs(title = "", x = "PC1 (71,0 %)", y = "PC2 (29,0 %)") +
  theme(legend.position = "bottom",
...
gg_pca_repro <- fviz_pca_biplot(pca_result, 
  ...
  labs(title = "", 
       x = "Componente Principal 1 (71,0 %)", 
       y = "Componente Principal 2 (29,0 %)") +
...
ggsave("mi_grafico_pca.tiff", plot = gg_pca_repro, device = "tiff", width = 10, height = 8, units = "in", dpi = 300)
...
# Guardar el gráfico como PNG con alta resolución
ggsave("mi_grafico_pca_year.tiff", plot = mi_grafico_pca_year, device = "tiff", width = 10, height = 8, units = "in", dpi = 300)
