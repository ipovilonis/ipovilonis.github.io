...
# Usamos la función para fusionar los datos
...
# Como se pierden datos usamos otra opción para combinar las tablas
...
library(ggplot2)
colores<-c("coral1","burlywood2","palegreen4")
ggplot(data=table_n_ovu, aes(x=year, y=Mean, fill=site))+
  stat_summary(fun = "mean", size = 1, geom = "bar") +
  geom_errorbar(aes(ymin=Mean-sd, ymax=Mean+sd),
                width=.2, position=position_dodge(.5))+
  facet_grid(.~site)+
...
library(ggplot2)
colores<-c("coral1","burlywood2","palegreen4")
ggplot(data=table_pol_mean, aes(x=year, y=Mean, fill=site))+
  stat_summary(fun = "mean", size = 1, geom = "bar") +
  geom_errorbar(aes(ymin=Mean-sd, ymax=Mean+sd),
                width=.2, position=position_dodge(.5))+
  facet_grid(.~site)+
