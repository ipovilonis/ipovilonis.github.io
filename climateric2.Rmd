---
title: "Respiration essay in Hexachlamys edulis"
output: pdf_document
---
...
gm_ajust <- ggplot(resp2, aes(x = time_min, y = carbon_ac)) +
  geom_point(aes(color = factor(matu)), size = 4)+
  geom_line(data = df_lineI, aes(x = x, y = y), color="green2", size= 0.8, linetype = "dotdash")+
  geom_line(data = df_lineM, aes(x = x, y = y), color="orange", size= 0.8, linetype = "dotdash")+
  labs(y = "CO2 (ml accumulated/Kg)", x = "Time (min)") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size = rel(0.5))) +
  scale_color_manual(values = colores) +
